...
function NeuralNetwork() {
  ...
  const particlesData = useMemo(() => [] as any[], []);
  ...
  useFrame((_, delta) => {
    if (!groupRef.current || !linesGeometryRef.current || !particlesRef.current || particlesData.length === 0) return;
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      for (let j = i + 1; j < particleCount; j++) {
        ...
        if (dist < minDistance) {
          ...
          const grayValue = alpha * 0.2; // Make it gray and dimmer
  ...
  return (
    <group ref={groupRef} dispose={null}>
      <points>
        ...
        <pointsMaterial color={'#404040'} size={3} blending={AdditiveBlending} transparent={true} sizeAttenuation={false} />
      </points>
      <lineSegments>
