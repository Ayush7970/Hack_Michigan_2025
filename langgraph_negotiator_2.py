...
async def on_negotiation_message(ctx: Context, sender: str, msg: Message):
    ...
    try:
        ...
        ctx.logger.info(f"🔄 Processing message through LangGraph - Round {current_state['negotiation_round'] + 1}")
        ctx.logger.info(f"📊 Current state status: {current_state['status']}")
        ...
        ctx.logger.info(f"🗣️ Sending response: {response_msg}")
