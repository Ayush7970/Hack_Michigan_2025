...
agent = Agent(
    ...
    endpoint=["http://localhost:8001/submit"],  # Enable HTTP endpoints
)
...
# ---------- events ----------
@agent.on_rest_post("/rest/post", request=InputMessage, response=BoolResponse)
async def on_start(ctx: Context, req: InputMessage) -> BoolResponse:
    ctx.logger.info(f"🔥 REST POST received - Agent: {agent.address}")
    ctx.logger.info(f"📨 Input: {req.input}")
    ctx.logger.info(f"🎯 Target address: {req.address}")

    if START_CONVO == "yes":
        # Add user message to conversation history
        conversation_history.append({"role": "user", "content": req.input})
        ctx.logger.info(f"📝 Added to conversation history. Total messages: {len(conversation_history)}")

        # short delay so mailbox is ready
        await asyncio.sleep(0.3)
        ctx.logger.info(f"🚀 Sending message to {req.address}: {req.input}")

        try:
            # Try sending via uagents first
            await ctx.send(req.address, Message(message=req.input))
            ctx.logger.info(f"✅ Message sent successfully!")
        except Exception as e:
            ctx.logger.error(f"❌ Failed to send message via uagents: {e}")

            # Fallback: try direct HTTP to local agent
            try:
                ctx.logger.info(f"🔄 Trying direct HTTP fallback to localhost:8000")
                import httpx
                async with httpx.AsyncClient() as client:
                    ...
                    ctx.logger.info(f"✅ Direct HTTP message sent! Status: {response.status_code}")
            except Exception as e2:
                ctx.logger.error(f"❌ Direct HTTP also failed: {e2}")
                return BoolResponse(success=False)

        return BoolResponse(success=True)

    ctx.logger.info(f"❌ START_CONVO is not 'yes', current value: {START_CONVO}")
    return BoolResponse(success=False)
...
async def get_conversation(ctx: Context) -> ConversationResponse:
    ...
    if len(conversation_history) >= 2:
        ...
        is_complete = check_negotiation_complete(last_message)
...
async def on_msg(ctx: Context, sender: str, msg: Message):
    ...
    conversation_history.append({"role": "seller", "content": msg.message})

    try:
        reply = await call_asi_one(ctx, msg.message)
        conversation_history.append({"role": "buyer", "content": reply})
        ctx.logger.info(f"Our response: {reply}")

        is_complete = check_negotiation_complete(reply)
        ctx.logger.info(f"Negotiation complete: {is_complete}")
